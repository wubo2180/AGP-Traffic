# import matplotlib
# 强制使用 Agg backend（非交互式），避免 CMD/PowerShell 差异
# matplotlib.use('Agg')
...
# Load the reconstructed and original data
    ...
    plt.plot(original_data, label='原始交通流量序列')
    plt.plot(reconstructed_data, label='重建交通流量序列')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.title('预训练交通流量序列重建')
    plt.xlabel('时间步')
    plt.ylabel('值')
    plt.legend()
    plt.savefig("./reconstruct.svg")
    plt.show()
...
def plot_test_vis():
    ...
    # Plot the time series
    plt.figure(figsize=(12, 6))
    plt.plot(original_data, label='真实值', linewidth=2, alpha=0.8)
    plt.plot(reconstructed_data, label='预测值', linewidth=2, alpha=0.8)
    ...
    try:
        pdf_file = "test_vis.pdf"
        svg_file = "test_vis.svg"
        
        plt.savefig(pdf_file, dpi=300, bbox_inches='tight')
        print(f"✓ PDF saved: {os.path.abspath(pdf_file)}")
        print(f"  File exists: {os.path.exists(pdf_file)}, Size: {os.path.getsize(pdf_file):,} bytes")
        
        plt.savefig(svg_file, bbox_inches='tight')
        print(f"✓ SVG saved: {os.path.abspath(svg_file)}")
        print(f"  File exists: {os.path.exists(svg_file)}, Size: {os.path.getsize(svg_file):,} bytes")
    ...
    # 使用 Agg backend 时不需要 show()，且 show() 可能导致问题
    # plt.show()
    plt.close()  # 关闭图形释放内存
...
if __name__ == "__main__":
    ...
    # except Exception as e:
    #     print(f"✗ Error: {e}")
    #     print("test_matplotlib.pdf could not be saved.")
    #     import traceback
    #     traceback.print_exc()
